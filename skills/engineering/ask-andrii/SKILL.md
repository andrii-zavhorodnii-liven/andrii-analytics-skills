...
- **`/to-experiment`** — spin up a sibling `<main>-experiments` repo for open-ended exploration that shouldn't pollute the main repo: a `uv`-managed Python project of plain `.py` scripts. Where **`/prototype`** answers *one* design question inside the main repo, this is for poking at data or an approach with no single question yet. Findings graduate back via `/to-tickets` (or `/to-spec` first, if big).
