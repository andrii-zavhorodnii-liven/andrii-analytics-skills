...
- **`/pr-merged`** — the tidy-up after one PR lands: fetch with prune, fast-forward the default branch, delete every `[gone]` local branch whose commits are contained in it. Anything unsafe — a branch checked out in a worktree, a squash/rebase merge the containment test cannot clear — is HELD, not deleted. For one merged PR; a pile-up of parallel worktrees is `/consolidating-work`'s job.
