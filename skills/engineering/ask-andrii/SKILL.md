...
- **`/pr-merged`** — the tidy-up after one PR lands: fetch with prune, fast-forward the default branch (checking it out only when this worktree is idle), delete every local branch whose remote is gone **and** whose commits are contained in the default branch. Branches checked out in a worktree, and branches the containment test cannot clear (squash/rebase merges), are reported as HELD and left standing. Remote branches, worktrees, and uncommitted work are untouched. A pile-up of parallel worktrees is `/consolidating-work`'s job.
